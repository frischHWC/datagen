...
public class Utils {
   ...
   * @param extension
   */
  public static void deleteAllLocalFiles(String directory, String name,
                                         String extension) {
    File folder = new File(directory);
    File[] files =
        folder.listFiles((dir, f) -> f.matches(name + ".*[.]" + extension));
    if (files != null) {
      for (File f : files) {
   ...
   * @param extension
   */
  public static void deleteAllHdfsFiles(FileSystem fileSystem, String directory,
  ...
  public static void createLocalDirectory(String path) {
    try {
      ...
      log.error("Unable to create directory of : " + path + " due to error: ",
          e);
    }
  }
  ...
  public static void recap(long numberOfBatches, long rowPerBatch,
    ...
    sinks.forEach(sink -> {
      ...
      case OZONE_PARQUET:
        ...
        break;
      case OZONE_ORC:
        log.info("   - Ozone as ORC files of " + rowPerBatch +
                " rows, in volume {} and bucket {} : ",
            model.getTableNames().get(OptionsConverter.TableNames.OZONE_VOLUME),
            model.getTableNames()
                .get(OptionsConverter.TableNames.OZONE_BUCKET));
        ...
        break;
      case OZONE_AVRO:
        log.info("   - Ozone as Avro files of " + rowPerBatch +
                " rows, in volume {} and bucket {} : ",
            model.getTableNames().get(OptionsConverter.TableNames.OZONE_VOLUME),
            model.getTableNames()
                .get(OptionsConverter.TableNames.OZONE_BUCKET));
        ...
        break;
      case OZONE_CSV:
        log.info("   - Ozone as CSV files of " + rowPerBatch +
                " rows, in volume {} and bucket {} : ",
            model.getTableNames().get(OptionsConverter.TableNames.OZONE_VOLUME),
            model.getTableNames()
                .get(OptionsConverter.TableNames.OZONE_BUCKET));
        ...
        break;
      case OZONE_JSON:
        log.info("   - Ozone as Json files of " + rowPerBatch +
                " rows, in volume {} and bucket {} : ",
            model.getTableNames().get(OptionsConverter.TableNames.OZONE_VOLUME),
            model.getTableNames()
                .get(OptionsConverter.TableNames.OZONE_BUCKET));
  ...
  public static String formatNumber(long numberToformat) {
    ...
    DecimalFormat thousandBillionFormatter =
        new DecimalFormat("### ### ### ### ###");
